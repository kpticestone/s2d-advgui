...
public class SwtComboBox<T> extends ASwtWidget<SelectBox<T>> {
    ...
    public SwtComboBox(ISwtWidget<? extends Group> pParent) {
        ...
        this.addDrawerBackground(new IInternalWidgetDrawer() {
            @Override
            public void drawIt(Batch pBatch, Vector2 pScreenCoords, Rectangle pDims) {
                bodr.setGenericColors(enabled, isFocused(), hovered);
                bodr.drawIt(pBatch, pDims);
                bodr.drawIt(pBatch, pDims.x+pDims.width-pDims.height, pDims.y, pDims.height, pDims.height);
            }
        });
    }
