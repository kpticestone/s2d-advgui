...
public class SwtCheckbox extends ASwtWidgetSelectable<CheckBox> {
    ...
    public SwtCheckbox(ASwtWidget<? extends Group> pParent, String text) {
        super(pParent);
        this.setText(text);
        TextureRegion check = context.getTextureRegion("ui2/check.png");
        ...
        this.addDrawerForeground(new IInternalWidgetDrawer() {
            ...
            public void drawIt(Batch pBatch, Vector2 pScreenCoords, Rectangle pDims) {
                ...
                borderDrawer.drawIt(pBatch, pDims.x, pDims.y, pDims.height, pDims.height);

                if (isChecked()) {
                    pBatch.draw(check, pDims.x + 6, pDims.y + 6, 11, 11);
                }

                a.draw(pBatch, SwtCheckbox.this.myText + " (" + isChecked() + ")", pDims.x + (pDims.height * 1.25f), pDims.y + (pDims.height) - (pDims.height - lh) / 2f);
            }
        });
    ...
    protected CheckBox _createActor() {
        ...
        cbs.checkboxOff = context.getDrawable("ui/slotType_minor.png");
        cbs.checkboxOff.setMinWidth(16);
        cbs.checkboxOff.setMinHeight(16);
        cbs.checkboxOff.setLeftWidth(100);
        cbs.checkboxOn = context.getDrawable("ui/icon.png");
