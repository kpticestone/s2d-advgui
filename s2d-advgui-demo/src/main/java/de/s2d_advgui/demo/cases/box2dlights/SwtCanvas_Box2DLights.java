...
public final class SwtCanvas_Box2DLights extends SwtCanvas<DemoResourceManager, SwtDrawerManager<DemoResourceManager>> {
    ...
    public SwtCanvas_Box2DLights(ISwtWidget<? extends Group> pParent,
        ...
        this.ligth = new PointLight(this.rayHandler, 64);
        this.ligth2 = new PointLight(this.rayHandler, 64);
    ...
    private void startAnim() {
        Rectangle src = new Rectangle(curLight.x, curLight.y, 1, 1);
        float newX = (float) ((Math.random() * 10) - 5);
        float newY = (float) ((Math.random() * 10) - 5);
        if (newX < -50) newX = -50;
        if (newX > 50) newX = 50;
        if (newY < -20) newY = -20;
        if (newY > 20) newY = 20;
        Rectangle dst = new Rectangle(newX, newY, 1, 1);
        Animation_ChangeBounds pAnimation = new Animation_ChangeBounds(0f, 5000f, src, dst,
                new IAnimationListener_ChangeBounds() {
                    @Override
                    public void onChange(Rectangle pRect) {
                        curLight.set(pRect.x, pRect.y);
                    }
                });
...
//        System.err.println(this.actor.getWidth() + "x" + this.actor.getHeight());
        ...
        try (SwtDrawer_Shapes sdr = this.drawerManager.startShapesDrawer()) {
            ...
            Collider colbo1 = new Collider(new Circle(curLight.x, curLight.y, .1f));
    ...
    public void printOth(ISwtDrawerManager<?> pDrawerManager, Vector2 pScreenCoords, Rectangle pDims) {

        pDrawerManager.getBatch().flush();
        ...
        this.ligth.setPosition(this.curLight.x, this.curLight.y);
        this.ligth.setDistance(10);
        this.ligth.setStaticLight(false);
        // ligth.setXray(false);

        this.ligth2.setColor(Color.RED);
        this.ligth2.setPosition(-6, 3);
        this.ligth2.setDistance(10);
...
//        this.theBody.setAwake(true);
        ...
        Batch pa = pDrawerManager.getBatch();
        pa.flush();
        pa.enableBlending();

        ShapeRenderer sr = pDrawerManager.getShapeRenderer();
        sr.flush();
    }
