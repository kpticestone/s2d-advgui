        ...
        this.addDrawerForeground(new InternalWidgetDrawerBatch() {
            ...
            protected void _drawIt(SwtDrawer_Batch<?> pBatch, Vector2 pScreenCoords, Rectangle pDims) {
                ...
                if (myIcon != null) {
                    ...
                    if (myText == null) {
                        pBatch.draw(check, pDims.x + (pDims.width - 16) / 2f, pDims.y + (pDims.height - 16) / 2f, 16,
                                16);
                    } else {
                ...
                if (myText != null) {
                    if (myIcon == null) {
                        ...
                        a.draw(pBatch.getBatch(), gl, pDims.x + (pDims.width - gl.width) / 2f,
                                pDims.y + pDims.height - (pDims.height - gl.height) / 2f);
                    } else {
                        a.draw(pBatch.getBatch(), SwtButton.this.myText, pDims.x + 24,
                                pDims.y + (pDims.height) - (pDims.height - lh) / 2f);
                    }
                }
            }
