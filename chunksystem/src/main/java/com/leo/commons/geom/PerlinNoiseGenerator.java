...
public class PerlinNoiseGenerator {
    ...
    public double improvedNoise(double x, double y, double z) {
        ...
        double c1 = grad(this.p_imp[aa], xo, yo, zo);
        double c2 = grad(this.p_imp[ba], xo - 1, yo, zo);
        double c3 = grad(this.p_imp[ab], xo, yo - 1, zo);
        double c4 = grad(this.p_imp[bb], xo - 1, yo - 1, zo);
        double c5 = grad(this.p_imp[aa + 1], xo, yo, zo - 1);
        double c6 = grad(this.p_imp[ba + 1], xo - 1, yo, zo - 1);
        double c7 = grad(this.p_imp[ab + 1], xo, yo - 1, zo - 1);
        double c8 = grad(this.p_imp[bb + 1], xo - 1, yo - 1, zo - 1);
     ...
     */
    private void initPerlin1(Random rnd) {
        ...
        for (i = 0; i < B; i++) {
            ...
            this.g1[i] = (float) (((rnd.nextDouble() * Integer.MAX_VALUE) % (B + B)) - B) / B;

            for (j = 0; j < 2; j++)
                this.g2[i][j] = (float) (((rnd.nextDouble() * Integer.MAX_VALUE) % (B + B)) - B) / B;
            normalize2(this.g2[i]);

            for (j = 0; j < 3; j++)
                this.g3[i][j] = (float) (((rnd.nextDouble() * Integer.MAX_VALUE) % (B + B)) - B) / B;
            normalize3(this.g3[i]);
        }

        while (--i > 0) {
            k = this.p[i];
            j = (int) ((rnd.nextDouble() * Integer.MAX_VALUE) % B);
            this.p[i] = this.p[j];
            this.p[j] = k;
        }
